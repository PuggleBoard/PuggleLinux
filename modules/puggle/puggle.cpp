...
static DefaultGUIModel::variable_t vars[] = {
	{ "System Status", "", DefaultGUIModel::STATE, },
	{ "IP Address", "", DefaultGUIModel::PARAMETER | DefaultGUIModel::COMMENT, },
	{ "Sampling Rate", "", DefaultGUIModel::PARAMETER | DefaultGUIModel::INTEGER, } ,
};

static size_t num_vars = sizeof(vars) / sizeof(DefaultGUIModel::variable_t);

Puggle::Puggle(void) :
	DefaultGUIModel("Puggle", ::vars, ::num_vars)
{
	QWhatsThis::add(this, "<p><b>Puggle:</b><br>RTXI plugin to interface with the PuggleBoard DAQ system.</p>");
...
void Puggle::execute(void)
{
	return;
}

void Puggle::update(DefaultGUIModel::update_flags_t flag)
{
	switch (flag) {
		case INIT:
			period = RT::System::getInstance()->getPeriod() * 1e-6; // ms
			ip_addr="192";																	// default using usb
			f_s=1000;																								// hz
			setComment("IP Address", ip_addr);											// default using usb
			setParameter("Sampling Rate", f_s);
			setState("System Status", state);												// setup function to get state from BBB
			break;
		case MODIFY:
			f_s = getParameter("Sampling Rate").toDouble();
			break;
		case UNPAUSE:
			break;
		case PAUSE:
...
void Puggle::createGUI(DefaultGUIModel::variable_t *var, int size)
{
	...
	QPushButton *connectBttn = new QPushButton("Connect", bttnGroup);
	QPushButton *disconnectBttn = new QPushButton("Disconnect", bttnGroup);
	...
	QObject::connect(connectBttn, SIGNAL(clicked()), this, SLOT(connect()));
	QObject::connect(disconnectBttn, SIGNAL(clicked()), this, SLOT(disconnect()));
...
void Puggle::connect(void)
{
	printf("Opening connection.\n");		// change this to print out as banner on module later
}

void Puggle::disconnect(void)
{
